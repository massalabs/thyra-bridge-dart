...
typedef FetchWebOnChain = ffi.Pointer<FetchWebOnChainReturn> Function(
    ffi.Pointer<Utf8>, ffi.Pointer<Utf8>);

class FetchWebOnChainReturn extends ffi.Struct {
...
class Binding {
  ...
  ffi.DynamicLibrary openLib() {
    ...
    if (Platform.isLinux) {
      return ffi.DynamicLibrary.open(absolute("native/$_libraryName.so"));
    }
    try {
  ...
  Uint8List fetchWoc(String address, String file) {
    final FetchWebOnChain nativeFunc = _library
        .lookup<ffi.NativeFunction<FetchWebOnChain>>('fetchWOC')
        .asFunction();
